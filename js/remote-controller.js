...
$("tour-stop").innerHTML=SLIDES.map((s,i)=>`<option value="${i}">${i+1}. ${s.title}</option>`).join("");
function message(text,error=false){$("connection-message").textContent=text;$("connection-message").style.color=error?"#ff9b9b":""}
function setConnected(on){$("connection-dot").style.background=on?"#43d17a":"#e0a844";$("headset-status").textContent=on?"המכשיר מחובר":"ממתינים למכשיר"}
function updateStatus(status={}){lastSeen=Date.now();setConnected(true);$("headset-mode").textContent=status.modeLabel||status.mode||"מחובר";if(Number.isInteger(status.tourIndex))$("tour-stop").value=String(status.tourIndex)}
async function connect(){
 ...
 try{session?.disconnect();session=new RemoteSession(token,{onStatus:updateStatus});await session.connect();history.replaceState(null,"","?session="+token);showControls();await send("controllerConnected");await session.send("request-status",{});message("השלט מחובר.")}
